---
name: columbus
description: Teaches how Columbus works as a local semantic code-context and durable memory system, with references for commands, retrieval, memory, work-item protocols, setup, and agent usage. Use when the user asks how Columbus works, wants to learn Columbus, needs Columbus command examples, asks about search, indexing, memory, epics, stories, tasks, navigator usage, or says "teach me Columbus".
---
...
2. **Explain the core model first**
   - Columbus has an indexed code layer for locating code and a durable knowledge layer for memory and work items.
   - `search`, `show`, and `graphs` retrieve code context.
   - `memory` records project knowledge, decisions, patterns, backlog, epics, stories, and tasks.
...
3. **Teach with a reference**
   ...
   - For durable memory and work tracking, read `references/memory-model.md`.
   - For how agents should claim, update, block, and complete epics, stories, and tasks, read `references/work-item-workflow.md`.
...
- [ ] The explanation distinguishes code index, memory, work items, and output modes.
