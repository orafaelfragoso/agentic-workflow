...
1. **Self-contained context** — everything needed is in the memory body: paths, code excerpts, conventions, commands.
...
Each plan is one Columbus `plan` memory. The template below is the `--body`; the anchors mirror the plan's content:

```sh
columbus memory add plan \
  --title "Plan: <imperative title — what will be true after this plan>" \
  --body "<the full template below, filled in>" \
  --tag improve --tag <category> \
  --link file:src/orders/api.ts \
  --evidence src/orders/api.ts:130-160
```

- **Tags**: always `improve` plus the category (`bug`, `security`, `perf`, `tests`, `tech-debt`, `migration`, `dx`, `docs`, `direction`).
- **Links**: every in-scope file, plus key symbols (`--link symbol:Name`).
- **Evidence**: the line ranges quoted in "Current state" — this is what `columbus memory validate` checks for drift.

The executor receives the plan either inlined in its brief or by id (`columbus show memory mem_NN --llm`). Either way the body must stand alone.

---

## Template (the memory body)

```markdown
# Plan: <Imperative title — what will be true after this plan>

> **Executor instructions**: Follow this plan step by step. Run every
> verification command and confirm the expected result before moving to the
> next step. If anything in the "STOP conditions" section occurs, stop and
> report — do not improvise. When done, report back to whoever dispatched
> you; the plan's owner updates Columbus memory, not you.
>
...
- **Effort**: S | M | L
- **Risk**: LOW | MED | HIGH
- **Depends on**: mem_NN — <plan title> (or "none")
...
- mem_13 requires mem_12 because <reason>.
...
- Could a model that has never seen this repo execute this with only the memory body and the repo? If any step requires knowledge from the advisor session, inline that knowledge.
...
- The memory's `--link` and `--evidence` anchors match the Scope and Current-state sections, and `columbus memory validate` passes after adding it.
