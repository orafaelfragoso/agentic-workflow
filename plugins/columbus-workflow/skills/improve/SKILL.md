...
- `reconcile` → process what happened since last session: verify executed plans and remove them (adding a fresh `documentation` memory only when something durable needs explaining), investigate blocked ones, refresh drifted plans, retire dead findings. See [references/plan-lifecycle.md](references/plan-lifecycle.md).
