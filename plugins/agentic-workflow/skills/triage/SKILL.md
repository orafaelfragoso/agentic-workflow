...
Cluster the items by what they're *about* — the feature or area they touch — not by tag. Use `columbus search "<intent>" --llm` to find the code and related memory each item points at; items that converge on the same area form one group. Tags inform the grouping but don't dictate it. A group is one coherent body of work that will become one epic.
