#!/usr/bin/env bash
# rtk-hook-version: 3
# RTK agent hook — rewrites commands to use rtk for token savings.
...
#   3 + stdout  Ask rule matched → rewrite but let the host prompt the user
...
case $EXIT_CODE in
  ...
  1)
    ...
    ;;
  2)
    # Deny rule matched — let the host's native deny rule handle it.
    exit 0
    ;;
  3)
    # Ask rule matched — rewrite the command but do NOT auto-allow so that
    # the host prompts the user for confirmation.
    ;;
  *)
    exit 0
    ;;
...
if [ "$EXIT_CODE" -eq 3 ]; then
  # Ask: rewrite the command, omit permissionDecision so the host prompts.
